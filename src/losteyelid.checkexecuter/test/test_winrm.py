...
class WinRMTest(unittest.TestCase):
	...
	def test_build_shell_request_xml(self):
		...
		self.assertTrue('<w:ResourceURI env:mustUnderstand="true">http://schemas.microsoft.com/wbem/wsman/1/windows/shell/cmd</w:ResourceURI>' in xml_str)
		self.assertTrue('<a:Action env:mustUnderstand="true">http://schemas.xmlsoap.org/ws/2004/09/transfer/Create</a:Action>' in xml_str)
	...
	def test_build_command_xml(self):
		winrm = WinRM()
		xml_obj = winrm.build_command_xml("SAMPLE-SHELL-ID", "SAMPLE-COMMAND", "SAMPLE-ARGUMENTS", "SAMPLE-MESSAGE-ID")
		xml_str = etree.tostring(xml_obj).decode("utf-8")
		self.assertTrue('<a:MessageID>uuid:SAMPLE-MESSAGE-ID</a:MessageID>' in xml_str)
		self.assertTrue('<w:OperationTimeout>PT60.000S</w:OperationTimeout>' in xml_str)
		self.assertTrue('<w:ResourceURI env:mustUnderstand="true">http://schemas.microsoft.com/wbem/wsman/1/windows/shell/cmd</w:ResourceURI>' in xml_str)
		self.assertTrue('<a:Action env:mustUnderstand="true">http://schemas.microsoft.com/wbem/wsman/1/windows/shell/Command</a:Action>' in xml_str)
	...
	def test_build_command_receive_xml(self):
		winrm = WinRM()
		xml_obj = winrm.build_command_receive_xml("SAMPLE-SHELL-ID", "SAMPLE-COMMAND-ID", "SAMPLE-MESSAGE-ID")
		xml_str = etree.tostring(xml_obj).decode("utf-8")
		self.assertTrue('<a:MessageID>uuid:SAMPLE-MESSAGE-ID</a:MessageID>' in xml_str)
		self.assertTrue('<w:OperationTimeout>PT60.000S</w:OperationTimeout>' in xml_str)
		self.assertTrue('<w:ResourceURI env:mustUnderstand="true">http://schemas.microsoft.com/wbem/wsman/1/windows/shell/cmd</w:ResourceURI>' in xml_str)
		self.assertTrue('<a:Action env:mustUnderstand="true">http://schemas.microsoft.com/wbem/wsman/1/windows/shell/Receive</a:Action>' in xml_str)
		self.assertTrue('<w:Selector Name="ShellId">SAMPLE-SHELL-ID</w:Selector>' in xml_str)
		self.assertTrue('<rsp:DesiredStream CommandId="SAMPLE-COMMAND-ID">stdout stderr</rsp:DesiredStream>' in xml_str)
	...
	def test_get_soap_header(self):
		...
		self.assertTrue('<w:ResourceURI env:mustUnderstand="true">SAMPLE-RESOURCE-URI</w:ResourceURI>' in xml_str)
